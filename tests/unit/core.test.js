...
describe("itinerary data integrity", () => {
  test("39 stops, 38 legs", () => {
    expect(stops.length).toBe(39);
...
describe("distances", () => {
  ...
  test("totalMiles is the sum of legs (~47,940)", () => {
    expect(totalMiles).toBe(legMiles.reduce((x, y) => x + y, 0));
    expect(totalMiles).toBeGreaterThan(46000);
    expect(totalMiles).toBeLessThan(50000);
  });
});
...
describe("CO2 model", () => {
  test("full tour is ~417 tonnes", () => {
    expect(totalMiles * CO2_PER_MILE).toBeCloseTo(417.1, 1);
  });
