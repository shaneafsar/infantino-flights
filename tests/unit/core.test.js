...
describe("distances", () => {
  ...
  test("totalMiles is the sum of legs (~28,407)", () => {
    expect(totalMiles).toBe(legMiles.reduce((x, y) => x + y, 0));
    expect(totalMiles).toBeGreaterThan(27000);
    expect(totalMiles).toBeLessThan(29000);
  });
});
...
describe("CO2 model", () => {
  test("full tour is ~247 tonnes", () => {
    expect(totalMiles * CO2_PER_MILE).toBeCloseTo(247.1, 1);
  });
