...
describe("itinerary data integrity", () => {
  test("33 stops, 32 legs", () => {
    expect(stops.length).toBe(33);
...
describe("distances", () => {
  ...
  test("totalMiles is the sum of legs (~38,898)", () => {
    expect(totalMiles).toBe(legMiles.reduce((x, y) => x + y, 0));
    expect(totalMiles).toBeGreaterThan(37000);
    expect(totalMiles).toBeLessThan(41000);
  });
});
...
describe("CO2 model", () => {
  test("full tour is ~338 tonnes", () => {
    expect(totalMiles * CO2_PER_MILE).toBeCloseTo(338.4, 1);
  });
