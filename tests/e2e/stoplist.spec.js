...
test("footer 'All stops' accordion is collapsed and lists the itinerary in order", async ({ page }) => {
  await page.goto("/");
  await expect(page.locator("circle.city")).toHaveCount(14); // app has rendered
  ...
  await expect(items.last()).toContainText("France 3–0 Iraq");          // latest stop
  await expect(items.last()).toContainText("Philadelphia");
