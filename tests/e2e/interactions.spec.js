...
test("end-of-tour stats", async ({ page }) => {
  ...
  await expect(page.locator("#miles")).toHaveText("47,938");
  await expect(page.locator("#games")).toHaveText("38");
  await expect(page.locator("#co2")).toHaveText("417.1");
  await expect(page.locator("#cost")).toHaveText("$1,302,512");
...
test("mi/km flips on a tap anywhere in the control (not just the off radio)", async ({ page }) => {
  ...
  await expect(page.locator("#miles")).toHaveText("77,149");
...
test("CO2 milestone text steps up across the tour", async ({ page }) => {
  ...
  await scrubToEnd(page); // ~417 t — lands on the Falcon 9 tier
  await expect(page.locator("#co2note")).toContainText("Falcon 9");
});
