...
const legMiles=[296,1291,313,822,2804,2732,966,1367,649,755,2459];
const totalMiles=legMiles.reduce((a,b)=>a+b,0);
const CO2_PER_MILE=3/1000;
const KM_PER_MILE=1.60934;
let unit="mi";
const toDisp=mi=>unit==="km"?mi*KM_PER_MILE:mi;
// CO2 milestones (cumulative tonnes -> relatable comparison)
const co2Steps=[
...
const milesEl=document.getElementById("miles"),co2El=document.getElementById("co2"),gamesEl=document.getElementById("games"),legEl=document.getElementById("leg"),milesLabelEl=document.getElementById("milesLabel"),co2NoteEl=document.getElementById("co2note");
...
const SPEED=0.012;
const PAUSE_FRAMES=240; // ~4s at 60fps
...
function render(){
  ...
  milesEl.textContent=Math.round(toDisp(miles)).toLocaleString();
  milesLabelEl.textContent=unit==="km"?"Km flown":"Miles flown";
  co2El.textContent=(miles*CO2_PER_MILE).toFixed(1);
  const co2t=miles*CO2_PER_MILE;
  let cs=-1;for(let i=0;i<co2Steps.length;i++){if(co2t>=co2Steps[i].t)cs=i;}
  if(cs!==co2Idx){const up=cs>co2Idx;co2Idx=cs;co2NoteEl.innerHTML=cs>=0?co2Steps[cs].m:"";if(up&&cs>=0){co2NoteEl.classList.remove("flash");void co2NoteEl.offsetWidth;co2NoteEl.classList.add("flash");}}

  // games attended = stops reached that were actual matches (Miami summit excluded)
  let reached=end?N:seg+(f>=1?1:0);
  let count=0;for(let i=0;i<=Math.min(reached,N);i++){if(i<stops.length&&stops[i].f1)count++;}
  gamesEl.textContent=count;
  ...
  if(atStop){dots[stopIdx].classList.add("current");labels[stopIdx].classList.add("current");
  ...
  if(end){
    const s=stops[N];
    legEl.innerHTML=vsHtml(s)+'<span class="dt">'+s.date+' &middot; '+s.n+(s.v?' &middot; '+s.v:'')+' &middot; '+Math.round(toDisp(totalMiles)).toLocaleString()+' '+unit+' total</span>';
